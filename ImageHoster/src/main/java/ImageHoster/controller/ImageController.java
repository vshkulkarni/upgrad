...
public class ImageController {
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        if (isOwner(imageId, session)) {
            ...
            return showImage(image.getTitle(), model);
        }
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, HttpSession session, RedirectAttributes redirectAttrs, Model model) {
        ...
        if (isOwner(imageId, session)) {
            ...
            model.addAttribute("delError", error);
            return showImage(image.getTitle(), model);
        }
    ...
    public String postComment(@PathVariable Integer imageId, @PathVariable String imageTitle, Comment comment, Model model, HttpSession session)
    {
        User user= (User)session.getAttribute("loggeduser");
        Image imageData=imageService.getImage(imageId);
        model.addAttribute("text",comment.getText());
        comment.setUser(user);
        comment.setImage(imageData);
        comment.setText(comment.getText());
        commentService.addComment(comment);
        model.addAttribute("text",comment.getText());
        return showImage(imageData.getTitle(), model);
    }

    private boolean isOwner(Integer imageId, HttpSession session) {
        Image imgData = imageService.getImage(imageId);
        Integer imageOwnerId = imgData.getUser().getId();
        User currentUser = (User) session.getAttribute("loggeduser");
        Integer currentUserId = currentUser.getId();
        return currentUserId == imageOwnerId ? true : false;
    }
}
